...
public class LocalDatabaseTopicData extends SQLiteOpenHelper {

    private static final String LOCAL_DATABASE_NAME = "topics";


    public LocalDatabaseTopicData(Context context) {
    ...
    public boolean addTopic(SubjectAdapter subjectAdapter) {
        SQLiteDatabase db = this.getWritableDatabase();
        ContentValues contentValues = new ContentValues();

        contentValues.put("_ID", subjectAdapter.getId());
        contentValues.put("name", subjectAdapter.getSubtopic());
        contentValues.put("pgno", subjectAdapter.getPgno());
        contentValues.put("keyname", subjectAdapter.getKeyname());
        contentValues.put("url", subjectAdapter.getUrl());
    ...
    public List<SubjectAdapter> getAll() {
        ...
        while (cursor.moveToNext()) {
            ...
            String pgno = cursor.getString(3);
            String url = cursor.getString(4);


            subjectAdapter.setId(id);
            subjectAdapter.setSubtopic(name);
            subjectAdapter.setKeyname(keyname);
            subjectAdapter.setPgno(pgno);
